...
// v1.2.7N - CSS 3D Rubik's Cube with internal state + full 6-face moves (no animation)
// - Face mapping adjusted: R->old F, F->old L, L->old B, B->old R (U/D unchanged)
// - D direction flipped (per earlier observation). B is WCA-standard in v1.2.7N.
...
(function () {
  ...
  // === 3D view rotation ===
  let rotX = -28; // FRU view
  let rotY = 55; // FRU view, R side ~20deg
  let dragging = false;
  let lastX = 0;
  let lastY = 0;
  ...
  // --- NOTE: D was flipped per earlier report. B is WCA-standard in v1.2.7N ---
...
function moveD() {
  ...
  function replaceLastHistoryToken(label) {
    const parts = stateMovesArea.value.trim().split(/\s+/).filter(Boolean);
    if (!parts.length) return;
    parts[parts.length - 1] = label;
    stateMovesArea.value = parts.join(" ");
  }
  ...
  // L -> old B
  // B -> old R
  const FACE_REMAP = { R: "R", F: "F", L: "L", B: "B", U: "U", D: "D" };
  ...
  function executeMove(face, prime) {
    ...
    if (mapped === "L") { if (prime) { moveL(); replaceLastHistoryToken("L\'"); } else { moveLprime(); replaceLastHistoryToken("L"); } return; }
    if (mapped === "B") { if (prime) { moveB(); replaceLastHistoryToken("B\'"); } else { moveBprime(); replaceLastHistoryToken("B"); } return; }
