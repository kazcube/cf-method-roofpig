...
// v1.2.13N - CSS 3D Rubik's Cube with internal state + full 6-face moves (no animation)
// - Face mapping adjusted: R->old F, F->old L, L->old B, B->old R (U/D unchanged)
// - D direction flipped (per earlier observation). B is WCA-standard in v1.2.13N.
...
function moveL'() {
  ...
  // --- NOTE: D was flipped per earlier report. B is WCA-standard in v1.2.13N ---
...
// === Force initial 3D orientation (v1.2.13N) ===
window.addEventListener('DOMContentLoaded', () => {
  const cubes = document.querySelectorAll('.cube3d');
  cubes.forEach(cube => {
    if (!cube.style.transform || cube.style.transform === '') {
